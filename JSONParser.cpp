	...
	bool JSONParser::parseElements(char aChar, JSONListener *aListener) {
		...
		switch (theType) {
			...
			case Element::quoted:
				...
				if (Element::object == theTop.type) {
					...
					if (kColon == input.peek()) {
						input.get(); //skip it...
						tempKey = temp;
					} else theResult = (!aListener) || aListener->addKeyValuePair(tempKey, temp);
				} else theResult = (!aListener) || aListener->addItem(temp);
				break;
			...
			case Element::constant:
				...
				if (Element::object == theTop.type) {
					theResult = (!aListener) || aListener->addKeyValuePair(tempKey, temp);
				} else theResult = (!aListener) || aListener->addItem(temp);
